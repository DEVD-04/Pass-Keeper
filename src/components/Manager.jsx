...
const Manager = () => {
    ...
    const savePassword = async () => {
        ...
        const newPassword = { ...form, password: encryptedPassword, id: form.id ? form.id : uuidv4() };
        
        // If editing, remove the old entry
        const updatedPasswords = form.id
            ? passwordArray.filter(item => item.id !== form.id)
            : [...passwordArray];
        
        setPasswordArray([...updatedPasswords, newPassword]);
        localStorage.setItem("password", JSON.stringify([...updatedPasswords, newPassword]));
    ...
    const editPassword = async (id) => {
        ...
        setForm({ ...passwordToEdit, password: decryptedPassword });
        const updatedPasswords = passwordArray.filter(item => item.id !== id);
        setPasswordArray(updatedPasswords);
    };
